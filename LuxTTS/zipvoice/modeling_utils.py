...
def process_audio(audio, transcriber, tokenizer, feature_extractor, device, target_rms=0.1, duration=None, feat_scale=0.1, prompt_text=None):
    if duration is None:
        prompt_wav, sr = librosa.load(audio, sr=24000, duration=None)
        max_dur = 15.0
        if len(prompt_wav) > int(max_dur * sr):
            prompt_wav = prompt_wav[:int(max_dur * sr)]
    else:
    ...
    if not prompt_text:
        ...
        if duration is None:
            ...
            max_dur = 15.0
            if len(prompt_wav2) > int(max_dur * sr2):
                prompt_wav2 = prompt_wav2[:int(max_dur * sr2)]
        else:
            prompt_wav2, sr2 = librosa.load(audio, sr=16000, duration=duration)
        prompt_text = transcriber(prompt_wav2)["text"]
    print(f"[LuxTTS] Prompt text: '{prompt_text}'", flush=True)
